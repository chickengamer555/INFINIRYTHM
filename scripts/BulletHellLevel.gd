...
var wall_push_force: float = 400.0  # How hard walls push the player (OOMPH!)
...
var speedup_multiplier: float = 3.0  # Speeds bullets to 3.0x speed (faster!)
var speedup_player_multiplier: float = 1.5  # Player moves at 2.0x speed (slower than bullets)
var speedup_audio_multiplier: float = 3.0  # Speeds audio to 3.0x speed (faster combo!)
...
var speedup_timer_increase_rate: float = 0.15  # Increases by 0.15 per second while holding (reduced penalty)
...
func update_player_input(delta):
	...
	if Input.is_action_pressed("speed_up") and speedup_key_released and speedup_timer <= 0:
		...
		# Boost combo rate significantly to reward the increased difficulty
		if GameManager:
			# Set time scale to match audio speedup
			GameManager.time_scale = speedup_audio_multiplier
			# Increase combo rate to 0.5 (5x faster) to reward the 3x difficulty
			# This gives effective rate of 0.5 * 3.0 = 1.5 per second (15x faster than normal!)
			GameManager.combo_rate = 0.5

	# While active, increase timer and keep time scale synced
	if is_speedup_active:
		# Increase timer by 0.2 per second while holding
		speedup_timer += speedup_timer_increase_rate * delta

		# Keep time scale and combo rate synced
		if GameManager:
			GameManager.time_scale = speedup_audio_multiplier
			GameManager.combo_rate = 0.5  # Maintain boosted combo rate

		# Deactivate only when Q released
		if not Input.is_action_pressed("speed_up"):
			is_speedup_active = false

			# Reset time scale and combo rate to normal
			if GameManager:
				GameManager.time_scale = 1.0
				GameManager.combo_rate = 0.1
	else:
		# When not active, ensure time scale and combo rate are normal
		if GameManager:
			GameManager.time_scale = 1.0
			GameManager.combo_rate = 0.1
...
func on_song_changed():
	...
	# NORMAL MODE: Heal 50 health when advancing to next song in playlist
	if GameManager:
		GameManager.heal(50)
