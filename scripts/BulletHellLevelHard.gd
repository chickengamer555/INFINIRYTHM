extends Control
class_name BulletHellLevelHard
...
var slowdown_drain_rate: float = 40.0  # Drain per second when active (5 seconds duration)
var slowdown_recharge_rate: float = 40.0  # Recharge per second when not active
var is_slowdown_active: bool = false
var slowdown_multiplier: float = 0.5  # Slows bullets to 20% speed (very slow)
...
var speedup_player_multiplier: float = 2.0  # Player moves at 2.0x speed (slower than bullets)
...
var speedup_timer_increase_rate: float = 0.2  # Increases by 0.2 per second while holding (reduced penalty)
...
var beat_cooldown: float = 0.06  # Slightly faster spawns for more bullets on screen
...
var peak_threshold_multiplier: float = 1.3  # Volume must be 1.5x average to be a peak (only loud peaks!)
var last_peak_time: float = 0.0  # Time of last peak detection
var peak_cooldown: float = 5.0  # Minimum 3.0 seconds between rotation reversals
...
var max_bar_width: float = 275.0  # Reasonable extension limit
...
func _process(delta):
	...
	# Update emitter rotation with dynamic direction (HARD MODE FEATURE!)
	emitter_rotation += delta * 0.3 * rotation_direction  # Direction changes on peaks!
...
func update_player_input(delta):
	...
	if Input.is_action_pressed("speed_up") and speedup_key_released and speedup_timer <= 0:
		...
		# Boost combo rate significantly to reward the increased difficulty (2.5x audio speed + 4x bullet speed)
		if GameManager:
			# Current multiplier = 1.0 + (combo_timer * 0.1)
			# We want same multiplier with new rate: 1.0 + (new_combo_timer * 0.8)
			# So: new_combo_timer = combo_timer * (0.1 / 0.8)
			GameManager.combo_timer = GameManager.combo_timer * (0.1 / 0.8)
			GameManager.combo_rate = 0.8  # 8x faster combo building to reward the extreme difficulty

	# While active, increase timer and combo rate
	if is_speedup_active:
		# Increase timer by 0.2 per second while holding
		speedup_timer += speedup_timer_increase_rate * delta

		# Ensure combo rate stays at 0.8x per second (8x faster than normal!)
		if GameManager:
			GameManager.combo_rate = 0.8
		...
		if not Input.is_action_pressed("speed_up"):
			...
			if GameManager:
				# Current multiplier = 1.0 + (combo_timer * 0.8)
				# We want same multiplier with new rate: 1.0 + (new_combo_timer * 0.1)
				# So: new_combo_timer = combo_timer * (0.8 / 0.1)
				GameManager.combo_timer = GameManager.combo_timer * (0.8 / 0.1)
...
func update_beat_detection(delta):
	...
	# Detect onsets in each band independently with lower thresholds
	var bass_onset = bass_flux > sensitivity * bass_flux_avg and bass_flux > 0.0005
	var mid_onset = mid_flux > sensitivity * mid_flux_avg and mid_flux > 0.0005
	var high_onset = high_flux > sensitivity * high_flux_avg and high_flux > 0.0005
...
func on_song_changed():
	...
	# HARD MODE: Heal 30 health when advancing to next song in playlist
	if GameManager:
		GameManager.heal(30)
